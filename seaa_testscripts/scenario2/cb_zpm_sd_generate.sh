...
# Pre-Req export Env/Ansible Variable to run Scenario
# ***********************************
export WORKING_DIR=${SEAA_CONFIG_PATH_TO_SE_ANSIBLE_ARTIFACTS}/../../../seaa_testscripts/scenario2
export INVENTORY_LOCATION=${WORKING_DIR}/inventory
export INVENTORY=cb_zpm_sd_singlehost.yaml
...
export ZPM_SMART_DEPLOY=true

# ***********************************
# Pre-Req export Env/Ansible Variable to run Scenario
# ***********************************
export ADMIN_NAMESPACES='"zstack-se-cb-zpm-sd-gen"'
...
extravars_file="${WORKING_DIR}/extravars/cb_zpm_sd_gen.json"
...
$run_step && echo "Admin - Deploy Admin Project and deploy SHARED Broker and ZPM"
$run_step && ${run_playbook_dir}/run-deploy-oel-dev-env.sh -v=${VERBOSITY} -t=${tags} -st=${skiptags} -i=${INVENTORY} -i_loc=${INVENTORY_LOCATION} \

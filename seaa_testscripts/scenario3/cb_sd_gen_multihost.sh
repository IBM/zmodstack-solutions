...
export PLAYBOOK_SCRIPT_DIR=${SEAA_CONFIG_PATH_TO_SE_ANSIBLE_ARTIFACTS}/../scripts/run_playbooks


# ***********************************
# Pre-Req export Env/Ansible Variable to run Scenario
# ***********************************
export WORKING_DIR=${SEAA_CONFIG_PATH_TO_SE_ANSIBLE_ARTIFACTS}/../../../seaa_testscripts/scenario3
export INVENTORY_LOCATION=${WORKING_DIR}/inventory
export INVENTORY=cb_sd_multihost.yaml
...
export ZOSCB_SMART_DEPLOY=true
...
# Pre-Req export Env/Ansible Variable to run Scenario
# ***********************************
export ADMIN_NAMESPACES='"zstack-se-cb-sd-gen-multi"'
...
extravars_file="${WORKING_DIR}/extravars/cb_sd_gen_multihost.json"
...
run_step=${DEPLOY}
# Admin - Deploy Admin Project and deploy SHARED Broker and endpoint
$run_step && echo "Admin - Deploy Admin Project and deploy SHARED Broker and create endpoint"
$run_step && ${PLAYBOOK_SCRIPT_DIR}/run-deploy-oel-dev-env.sh -v=${VERBOSITY} -t=${tags} -st=${skiptags} -i=${INVENTORY} -i_loc=${INVENTORY_LOCATION} \
